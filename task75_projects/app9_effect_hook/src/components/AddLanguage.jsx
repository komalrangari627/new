...
const AddLanguage = (props) => {
  ...
  const checkTitle = (e) => {
    try {
      ...
      const result = props.dataSetLanguages.filter(
        (language) => value.toLowerCase() === language.title.toLowerCase()
      );
      if (result.length !== 0) throw new Error("Duplicate entry!");
  ...
  return (
    <div className="add-language-form">
      ...
      >
        <input
          className="p-2 border border-x-gray-600 rounded-lg outline-none focus:ring-2 focus:ring-blue-400 transition"
          onChange={(event) => {
            handleChange(event);
            checkTitle(event);
          }}
        ...
        {!checkUniqueTitle && (
          <h1 className="font-bold p-2 bg-red-400 text-white">
